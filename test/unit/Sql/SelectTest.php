...
class SelectTest extends TestCase
{
    ...
    public function testWhereArgument1IsString()
    {
        ...
        $where      = $select->getRawState('where');
        $predicates = $where->getPredicates();
        self::assertInstanceOf(Predicate\Literal::class, $predicates[0][1]);
    }
    ...
    public function testWhereArgument1IsAssociativeArrayNotContainingReplacementCharacter()
    {
        ...
        self::assertInstanceOf(Predicate\Operator::class, $predicates[0][1]);
        self::assertEquals(Where::OP_AND, $predicates[0][0]);
        self::assertEquals('name', $predicates[0][1]->getLeft());
        self::assertEquals('Ralph', $predicates[0][1]->getRight());

        self::assertInstanceOf(Predicate\Operator::class, $predicates[1][1]);
    ...
    public function testOrder()
    {
        ...
        $select = new Select();
        $select->order(
            $this->getMockBuilder(Predicate\Operator::class)
                ->setMethods()
