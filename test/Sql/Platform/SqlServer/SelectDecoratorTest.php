...
class SelectDecoratorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testPrepareStatement(Select $select, $expectedSql, $expectedParams)
    {
        ...
        $adapter = $this->getMock(
            ...
            null,
            array(
                $this->getMock('Laminas\Db\Adapter\Driver\DriverInterface'),
    ...
    public function testGetSqlString(Select $select, $notUsed, $notUsed, $expectedSql)
    {
        ...
        $adapter = $this->getMock(
            'Laminas\Db\Adapter\Adapter',
            null,
            array($this->getMock('Laminas\Db\Adapter\Driver\DriverInterface'))
        );
        $parameterContainer = new ParameterContainer;
        $statement = $this->getMock('Laminas\Db\Adapter\Driver\StatementInterface');
        $statement->expects($this->any())->method('getParameterContainer')->will($this->returnValue($parameterContainer));
    ...
    public function dataProvider()
    {
        ...
        $expectedPrepareSql1 = 'SELECT [bar], [bam] FROM ( SELECT [foo].[bar] AS [bar], [foo].[baz] AS [bam], ROW_NUMBER() OVER (SELECT 1) AS [__LAMINAS_ROW_NUMBER] FROM [foo] ) AS [LAMINAS_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [LAMINAS_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__LAMINAS_ROW_NUMBER] BETWEEN ?+1 AND ?+?';
        $expectedParams1 = array('offset' => 10, 'limit' => 5, 'offsetForSum' => 10);
        $expectedSql1 = 'SELECT [bar], [bam] FROM ( SELECT [foo].[bar] AS [bar], [foo].[baz] AS [bam], ROW_NUMBER() OVER (SELECT 1) AS [__LAMINAS_ROW_NUMBER] FROM [foo] ) AS [LAMINAS_SQL_SERVER_LIMIT_OFFSET_EMULATION] WHERE [LAMINAS_SQL_SERVER_LIMIT_OFFSET_EMULATION].[__LAMINAS_ROW_NUMBER] BETWEEN 10+1 AND 5+10';
